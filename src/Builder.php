...
class Builder
{
    const SAGEPAY_ENDPOINT = 'https://%s.sagepay.com/gateway/service/vspform-register.vsp';
    ...
    // output variables
    public $sagePayResponse;

    /**
     * Create a new SagePayForm Instance.
     */
    ...
    {
        $environment = ($this->settings['isProduction'] ? 'live' : 'test');

        return sprintf(self::SAGEPAY_ENDPOINT, $environment);
    }
    ...
    protected function encryptAndEncode($strIn)
    {
        ...
        // use "phpseclib/mcrypt_compat" package
        $mcrypt = phpseclib_mcrypt_module_open(MCRYPT_RIJNDAEL_128, '', MCRYPT_MODE_CBC, '');

        phpseclib_mcrypt_generic_init($mcrypt, $this->settings['encryptPassword'], $this->settings['encryptPassword']);

        $cipher_text = phpseclib_mcrypt_generic($mcrypt, $strIn);
    ...
    protected function decodeAndDecrypt($strIn)
    {
        ...
        return phpseclib_mdecrypt_generic($mcrypt, $strIn);
    }
    ...
    public function decode($strIn)
    {
        ...
        $this->sagePayResponse = $sagePayResponse;

        return $this->sagePayResponse;
    }

    public function isResponseValid()
    {
        return isset(
            $this->sagePayResponse['Status'],
            $this->sagePayResponse['StatusDetail'],
            $this->sagePayResponse['VendorTxCode'],
            $this->sagePayResponse['Amount']
        );
    }

    protected function getResponseStatus()
    {
        return $this->sagePayResponse['Status'];
    }
    ...
    public function getResponseStatusMessage()
    {
        $outcomes = [
            ...
            'NOTAUTHED' => 'completed, but was not authorised',
            'MALFORMED' => 'failed because of a malformed submission',
            'INVALID' => 'failed because of an invalid submission value',
            'ABORT' => 'failed because you cancelled the transaction',
            'REJECTED' => 'failed because of fraud checks',
            'ERROR' => 'failed because of an error',
        ];
    ...
    {
        return trim( $this->getResponseTransactionID(), '{}' );
    }

    public function getResponseAmount()
    {
        return $this->sagePayResponse['Amount'];
    }
